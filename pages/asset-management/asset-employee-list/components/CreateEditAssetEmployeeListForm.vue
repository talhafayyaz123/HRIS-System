...
onMounted(async function () {
  try {
    ...
    suppliersOptions.value = softwareRes?.data?.map((ps: any) => {
      return {
        value: ps.id,
        text: ps.name,
      };
...
onMounted(async function () {
  try {
    ...
    if (!!response?.success) {
      const { assetName, suppliers, storeArticle, assetType, priceNetto } =
        response?.data;
              ...
              <div class="col-lg-6">
                <div class="form-group">
                  <elements-multiselect
                    ...
                    :required="true"
                    :class="{ 'is-invalid': v$.suppliers.$error }"
                  />
                </div>
                <div v-if="v$.suppliers.$error" class="invalid-feedback">
                  <span class="text-danger" v-if="v$.suppliers.required">{{
                    $t("This value is required")
                  }}</span>
                </div>
              </div>
