...
const { router, Toast } = commonCode(),
  ...
  update = () => {
    ...
    if (v$.value.$invalid) {
      ...
      updateFunc(state, AMSId, "/support/ams");
    }
  },
  refresh = async () => {
    const layout = useLayoutStore();
    layout.changeLoaderValue(true);
    let response = await showFunc(AMSId);
    if (!!response?.success) {
      let resp = response?.data;
      state.customerId = resp?.customerId?.id;
      state.serviceHoursYear = resp?.serviceHoursYear;
      state.remainingHours = resp?.usedHours;
      state.hourlyRate = resp?.hourlyRate;
      state.monthlyAmount = resp?.monthlyAmount;
    }
...
<template>
  <div>
    ...
    <form @submit.prevent="update">
      ...
      <div class="mt-3">
        <submittal-button :isLoading="submitted" :buttonName="$t('Update')" />
      </div>
    </form>
  </div>
</template>
