...
import {
  ...
  onPremiseUpdateRequest,
  onPremiseShowRequest,
...
const { router, Toast } = commonCode(),
  ...
  { title } = useTitle("Edit On-Premise");
const store = () => {
  v$.value.$touch();
  if (v$.value.$invalid) {
...
async function refresh() {
  ...
  let operatingSystemListing = await showGenericListing(
    routes.OPERATING_SYSTEM
  );
  ...
  onPremiseShowRequest(onPremiseId).then((response) => {
    state.softwareId = response?.data?.softwareId;
    versionsList(response?.data?.softwareId);
    ...
    state.versionId = response?.data?.softwareVersionId;
  });
  layout.changeLoaderValue(false);
