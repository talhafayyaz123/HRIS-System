...
const { showGenericListing } = genericFeatures(),
  { submitted, isLoading, apiRoute, showFunc, updateFunc } =
  ...
  store = () => {
    ...
    if (v$.value.$invalid) {
      ...
      updateFunc(payload, hostingId, "/system/hosting");
    }
  },
  validationRules = {
    customerId: { required },
...
async function getHosting() {
  ...
  let response = await showFunc(hostingId);
  if (!!response?.success) {
    state.customerId = response?.data?.customerId;
    state.instanceType = response?.data?.instanceType;
    state.operatingSystem = response?.data?.operatingSystemId;
    state.host = response?.data?.systemHostId;
    state.systemName = response?.data?.systemName;
    state.hostingType = response?.data?.hostingType;
    ...
    if (state.hostingType == "vm") {
      stateVm.software = response?.data?.softwareId;
      getVersions(response?.data?.softwareId);
      stateVm.host = response?.data?.host;
      stateVm.username = response?.data?.username;
      stateVm.serverPort = response?.data?.serverPort;
      stateVm.password = response?.data?.password;
      stateVm.version = response?.data?.softwareVersionId;
    }
    if (state.hostingType == "docker") {
      stateDocker.databaseDiskSize = response?.data?.diskSize;
      stateDocker.language = response?.data?.systemLanguage;
      stateDocker.size = response?.data?.systemSize;
      stateDocker.url = response?.data?.url;
    }
  }
