...
const props = defineProps(["ams", "openAccordions"]);
const emits = defineEmits(["toggle-accordion", "edit-entry"]);
const amsRecord = ref([]);

amsRecord.value =props.ams;
...
const recordOrderChanged = async (event: any) => {
  ...
  if (event.moved) {
    ...
    movedEntry = amsRecord.value[movedIndex];
    const previousEntry = amsRecord.value[movedIndex - 1];
    const nextEntry = amsRecord.value[movedIndex + 1];
