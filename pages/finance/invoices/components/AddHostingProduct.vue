  ...
  if (e.target.checked) {
    const singleProd = {
      ...
      totalNetto:
        ...
        (product.discount / 100) * (product.quantity * product.pricePerPeriod),
      totalRate: product.quantity * product.pricePerPeriod, // that of child
      additionDescription: "",
  ...
  addedProducts.value.map((addedProd, index) => {
    if (addedProd.productId === productId) {
      ...
      addedProducts.value[index].totalNetto =
        changedQuantity * addedProducts.value[index].pricePerPeriod -
        (addedProducts.value[index].discount / 100) *
          (changedQuantity * addedProducts.value[index].pricePerPeriod);
      addedProducts.value[index].totalRate =
        changedQuantity * addedProducts.value[index].pricePerPeriod; // for child
      return;
    }
  });
};
