...
const
    ...
    { submitted, isLoading, apiRoute, storeFunc, showFunc } = genericFunctionality(),
    state = reactive({
        processNumber: "",
        title: "",
        date: new Date(),
    ...
    update = () => {
        ...
        if (v$.value.$invalid) {
            ...
            formData.append("employeeId", props.userId);
            ...
            apiRoute.value = routes.CIP+'/update/'+props.cIPId;
            storeFunc(formData)
            //$event('cIPListRefresh', true)
        }
    },

    refresh = async () => {
        let response = await showFunc(props.cIPId)
        if(!!response?.success){
            const data = response?.data
            state.processNumber = data?.processNumber
            state.title = data?.title
            state.date = data?.date
            state.description = data?.description
            state.suggestedSolution = data?.suggestedSolution
            //state.affectedGroup = data?.affectedGroup
        }
    }
...
<template>
    ...
    <form  @submit.prevent="update">
        ...
        <div class="mt-3 d-flex align-items-center justify-content-end">
            <submittal-button :isLoading="isLoading" :icon-name="'saveIcon'" :buttonName="$t('Save and Proceed')" />
        </div>
    </form>
</template>