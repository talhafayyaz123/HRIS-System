...
function addProduct(e, product, productId) {
  if (e.target.checked) {
    const singleProd = {
      ...
      productPrice: Number(product.quantity) * Number(product.salePrice)
    };
...
const changeQuantity = (event, productId, changeType?: any) => {
  ...
  addedProducts.value.map((addedProd, index) => {
    if (addedProd.productId === productId) {
      ...
      // sale price
      addedProducts.value[index].productPrice = Number(changedQuantity) * Number(addedProducts.value[index].salePrice);
      return;
    }
  });
};
