<template>
  <div>
    <!-- 
         ...
         -->
    <div :style="`position:fixed;top:40%;z-index:999; right: ${configurationToggle ? '29.3' : '0'
      }%;`" class="Widget-Config" @click="configurationToggle = !configurationToggle">
      <div class="widget-config-btn">
        <div class="icon-bg">
          <ElementsIcon v-bind:icon="'settingConfigIcon'"></ElementsIcon>
        </div>
        <div class="d-flex align-items-center content-text">
          <p class="me-2">Setting</p>
          <ElementsIcon v-bind:icon="'saveIcon'"></ElementsIcon>
        </div>
      </div>
    </div>
    <!-- 
    ...
    <!-- 
          ...
          -->
    <div v-if="configurationToggle" class="widget-configuration">
      <!-- 
      ...
      <div class="widget-divider"></div>
      <div class="style-overflow" v-if="selectedWidget && reloadConfig">
        <div class="widget-card mt-0">
          <div class="widget-card-body">
            <div v-if="selectedWidget?.type === 'card'">
              <TextInput @change="labelInputChanged($event, 'card')" :modelValue="selectedWidget.configuration.heading"
                type="text" :label="$t('Heading')" class="pb-2 w-full" />
            </div>
            <div class="" v-if="!['row', 'column', 'card'].includes(selectedWidget?.type)">
              <TextInput @change="labelInputChanged($event, 'widget')" type="text"
                :modelValue="selectedWidget.configuration.label" :label="$t('Label')" class="pb-2 w-full" />
            </div>
            <div class="" v-if="selectedWidget?.type === 'text-input' ||
              selectedWidget?.type === 'number-input'
              ">
              <TextInput v-model="selectedWidget.configuration.placeholder" type="text" :label="$t('Placeholder')"
                class="pb-2 w-full" />
            </div>
            <div class="" v-if="showForAllInputWidgets">
              <TextInput v-model="selectedWidget.configuration.name" type="text" :label="$t('Name')"
                class="pb-2 w-full" />
              <!-- variable in the config file which will be replaced with the value of the widget -->
              <div class="mb-2">
                <label class="form-label" :for="id"><span v-if="required" style="color: red">*</span>&nbsp;{{
                  $t("Config Variable Name")
                }}:</label>
                <input @change="setConfigVariableName" @input="checkConfigVariableName"
                  :value="selectedWidget.configuration.configVariableName" class="p-2 w-full form-input" type="text" />
              ...
              <!-- readonly fields which will display the value as a visual aid for the user, indicating the value which will
                    ...
                    -->
              <TextInput v-model="selectedWidget.configuration.value" type="text" :label="$t('Value')" :isReadonly="true"
                class="pb-2 w-full" />
            </div>
            <!-- 
                 ...
                 -->
            <div class="flex flex-col mb-2" v-if="selectedWidget?.type === 'select'">
              <label>{{ $t("Options") }}:</label>
              <div v-for="(option, index) in selectedWidget.configuration.options" :key="'option-' + index"
                class="flex gap-1 p-2 mb-2 card rounded shadow relative">
                <font-awesome-icon @click="removeOption(index)" class="cursor-pointer text-black absolute right-2"
                  icon="fa-solid fa-xmark" />
                <TextInput v-model="option.name" type="text" :label="$t('Name')" class="pb-2 w-1/2" />
                <TextInput v-model="option.value" type="text" :label="$t('Value')" class="pb-2 w-1/2" />
              </div>
              <button @click="addOption" class="py-2 px-3 mt-2 rounded docsHeroColorBlue">
                {{ $t("Add Option") }}
              </button>
            </div>
            <!-- 
            ...
            <div class="flex flex-col mb-2" v-if="selectedWidget?.type === 'table'">
              ...
              <div v-for="(field, index) in selectedWidget.configuration.fields" :key="'field-' + index"
                class="flex gap-1 p-2 mb-2 card rounded shadow relative">
                <font-awesome-icon @click="removeField(index)" class="cursor-pointer text-black absolute right-2"
                  icon="fa-solid fa-xmark" />
                <TextInput v-model="field.displayName" type="text" :label="$t('Display Name')" class="pb-2 w-1/2" />
                <TextInput v-model="field.variableName" type="text" :label="$t('Variable Name')" class="pb-2 w-1/2" />
              </div>
              <button @click="addField" class="py-2 px-3 mt-2 rounded docsHeroColorBlue">
                {{ $t("Add Field") }}
              </button>
            </div>
            ...
            <div class="mb-2" v-if="selectedWidget?.type === 'checkbox'">
              <TextInput v-model="selectedWidget.configuration.inputGroupName" type="text" :label="$t('Input Group Name')"
                class="pb-2 w-full" />
            </div>
          </div>
        </div>
        ...
        <div class="mb-2" v-if="selectedWidget?.type === 'column'">
          <TextInput :isReadonly="selectedWidget.configuration.equallySizedColumns" type="number"
            v-model="selectedWidget.configuration.inlineCSS.width" min="50" :label="$t('Width')" class="pb-2 w-full" />
        </div>
        ...
        <div class="mb-2" v-if="selectedWidget?.type === 'row'">
          <div class="flex justify-between">
            ...
            <input @input="sizeColumnsEqually(selectedWidget)" :checked="selectedWidget.configuration.equallySizedColumns"
              v-model="selectedWidget.configuration.equallySizedColumns" type="checkbox" id="equal-columns" />
          </div>
        </div>
      </div>
