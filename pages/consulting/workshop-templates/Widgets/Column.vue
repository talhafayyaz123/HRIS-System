<template>
  <div @click.stop="selectColumn" @dragenter="onDragenter" @dragover="onDragover" @drop="onDrop" @mouseenter="addToolbar"
    @mouseleave="removeToolbar" :class="[
      'template-column',
      'position-relative',
      showPreview ? '' : 'p-2 column-bg-color',
  ...
  minHeight: column?.widgets?.length || showPreview ? '' : '70px', //If there are no widgets in a column, add a min-height to the column to make it visible
    ...
    <!-- 
         ...
         -->
    <slot name="toolbar" :showToolbar="showToolbar" :selectColumn="selectColumn"></slot>
    <draggable class="flex flex-col" v-model="column.widgets" group="widgets" :move="() => !showPreview" :handle="handle"
      item-key="id">
      <!-- the widget is dynamically loaded based on the type -->
      <template #item="{ element: widget, index }">
        <span @mouseenter="addHandleIfWysiwyg(widget)" @mouseleave="handle = ''">
          <!-- 
                        only add the toolbar hanle icon if the widget is of type 'wysiwyg'
                     -->
          <font-awesome-icon v-if="widget.type === 'wysiwyg' && handle" class="cursor-pointer wysiwyg-handler absolute"
            icon="fa-solid fa-up-down-left-right" />
          <component @removeWidget="removeWidget(index)" :is="widget.type" :widget="widget" :id="widget.id" :key="index">
          </component>
        </span>
      </template>
    </draggable>
...
import {
  workshopTemplateWidgetCreateRequest,
  workshopTemplateWidgetDeleteRequest,
  ...
  methods: {
    ...
    setInputConfigurations(widgets) {
      ...
      widgets.forEach((widget) => {
        ...
        if (widget.type === "checkbox" && widget.configuration.inputGroupName) {
          ...
          if (
            !inputConfigurations["groups"][widget.configuration.inputGroupName]
          )
            inputConfigurations["groups"][widget.configuration.inputGroupName] =
              "{}";
          inputConfigurations["groups"][widget.configuration.inputGroupName] =
      ...
      const workshop = useWorkshopTemplateStore();
      workshop.setInputConfiguration(inputConfigurations);
    },
    /**
    ...
    selectColumn() {
      if (!this.showPreview) {
        if (this.widget) {
          const workshop = useWorkshopTemplateStore();
          workshop.setSelectedWidget(this.widget);
          workshop.setSelectedWidgetType(this.widget.type);
        }
      }
    },
    /**
        ...
        if (this.isDragging) {
          ...
          const widget = {
            ...
            configuration: {
              ...
              styleClasses: {
                fontSize: widgetType === "heading" ? "text-xl" : "text-base",
          ...
          workshopTemplateWidgetCreateRequest({
            workshopTemplateColumnId: this.column.id,
            ...
            .then((response) => {
              ...
              this.column.widgets = [...this.column.widgets, { ...widget }];
            })
            .catch((error) => {
    ...
    async removeWidget(index) {
      try {
        ...
        workshopTemplateWidgetDeleteRequest(this.column.widgets[index]?.id);
