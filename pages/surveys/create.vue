<template>
  <div style="word-break: break-word">
    ...
    <div
        ...
        style="width: 22%; right: 0%; height: 100%; z-index: 1"
    >
      <h5
...
import {
  ...
  surveyChapterPutRequest, surveySettingsUpdateRequest, surveyStyleConfigurationsPostRequest
} from "~/utils/apiRequests";
import {mapWritableState} from "pinia";
import AddSoftwareProductsModal from "~/components/Survey/AddSoftwareProductsModal.vue";
...
export default {
  ...
  methods: {
    ...
    async cartConfigToggled(event, type) {
      ...
      // Update or create survey settings 
      const surveyData = {
        isManualProduct: type === "addProductsManually" ? event.target.checked : this.cart.addProductsManually,
        minimizeCart: type === "minimizeCart" ? event.target.checked : this.cart.minimizeCart,
        minimizeStep: type === "minimizeSteps" ? event.target.checked : this.cart.minimizeSteps,
      };

      if (this.surveyId) {
        const response = await surveySettingsUpdateRequest(surveyData, this.surveyId);
      } else {
        // const response = await surveySettingsUpdateRequest(surveyData);
      }

      // Setting survey style configurations
    ...
    async save() {
      try {
        ...
        for (let i = 0; i < this.steps.length; i++) {
          ...
          try {
            const itemType = this.steps[i].type === "chapter" ? "chapters" : "questions";
            const itemId = payload?.value?.id ?? payload?.id;
            const itemData = payload?.value ?? payload;
          ...
          if (this.surveyId || this.$route.params.id) {
            await surveySettingsUpdateRequest({
              isManualProduct: this.cart.addProductsManually ? 1 : 0,
        ...
        const surveyId = this.$route.params.id || this.surveyId;

        const configurations = this.cart.config.map((config) => ({
          ...config,
          surveyId: surveyId,
        }));

        console.log("CHecking the conffff::", configurations);
        ...
        // await surveyConfigurationsPostRequest({
        //   configurations: this.cart.config.map((config) => {
        //     return {
        //       ...config,
        //       surveyId: this.$route.params.id ?? this.surveyId,
        //     };
