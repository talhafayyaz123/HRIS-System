<template>
  <div style="word-break: break-word">
        ...
        <div>
          ...
          >
            <div
                ...
                v-if="
                                selectedQuestion?.groups[0]?.type ===
                                'image-input'
                            "
            >
              <p>
            ...
            <div v-else class="tabs">
              <div
                  v-for="(group, groupIndex) in selectedQuestion.groups"
    ...
    <div class="grid grid-cols-[5fr,2fr] mb-2 header">
    ...
    <div
        ...
        :style="`right: ${
                styleBarToggle ? '22' : '0'
            }%; top: 50%; background: #ed7d31; z-index: 1; width:55px; float:right`"
        class="absolute card px-3 py-2 bg-white-500 rounded shadow-md cursor-pointer"
        @click="styleBarToggle = !styleBarToggle"
    >
      <ElementsIcon
          color="#2957a4"
          v-bind:icon="'projectManIcon'"
      />
    </div>
    <div
        ...
        :style="`right: ${
                cartToggle ? '30' : '0'
            }%; top: 70%; background: #ed7d31; z-index:1`"
        class="absolute card px-3 py-2 bg-white-500 rounded shadow-md cursor-pointer"
...
<!--  Product Modals-->
  <add-software-products-modal
      v-if="productToggle"
      :showModal="productToggle"
  ...
  <add-software-products-modal
      ...
      :showModal="numberInputProductsToggle"
      @productAdded="addNumberInputProducts"
      :option="option"
      :formulaInput="true"
  ...
  <add-software-products-modal
      ...
      :showModal="conditionsProductToggle"
      @productAdded="addConditionsProduct"
      :formulaInput="
...
import {
  genericListingRequest, surveyChapterPutRequest, surveyConfigurationsPostRequest, surveyQuestionPutRequest,
  surveySettingsUpdateRequest,
  surveyShowGetRequest,
...
export default {
  ...
  async mounted() {
    ...
    // });
    try {
      const response = await genericListingRequest(routes.PRODUCT, {
        type: "software",
      });
  ...
  computed: {
    ...mapWritableState(useSurveysStore, {
      "surveyId": "id",
      "errorList": "errorList",
    }),
    filteredQuestion() {
  ...
  methods: {
    ...
    removeSelectedQuestionConfigurationType() {
      this.selectedQuestion.groups[0].type = "";
    },
    /**
    ...
    setSelectedQuestionType(type) {
      if (type === "image-input") {
        this.selectedQuestion.groups[0].type = type;
        this.selectedQuestion.groups.splice(
            0,
            this.selectedQuestion.groups.length
        ); // remove all the options added for the selected question
        this.selectedQuestion.conditions.splice(
            0,
            this.selectedQuestion.conditions.length
    ...
    async cartConfigToggled(event, type) {
      ...
      const response = await surveySettingsUpdateRequest({
        addProductsManually:
    ...
    async refresh() {
      this.questions = [];
      this.steps = [];
      await surveyShowGetRequest(this.$route.params.id)
          .then((res) => {
            this.cart.addProductsManually =
                res?.data?.isManualProduct == 1;
            this.cart.minimizeCart = res?.data?.minimizeCart == 1;
            this.cart.minimizeSteps = res?.data?.minimizeStep == 1;
            this.minimizeCart = res?.data?.minimizeCart == 1;
            this.minimizeSteps = res?.data?.minimizeStep == 1;
            this.stylesConfiguration = res?.data?.styleConfiguration;
            
            this.cart.config = res?.data?.configurations;
            ...
            questionsAndChapters.forEach((questionOrChapter) => {
              if (questionOrChapter.questions !== undefined) {
                ...
                questionOrChapter.groups.forEach(
                    (group) => {
                      group.options.forEach((option) => {
                        this.addOptionMixin(option);
                      });
                ...
                questionOrChapter.conditions.forEach(
                    (condition) => {
                      this.addConditionMixin(condition);
                    }
                );
            ...
            questionsAndChapters.forEach((questionOrChapter) => {
              if (questionOrChapter.questions !== undefined) {
                let questionStep = {
                  ...
                  type: "chapter",
                  value: {
                    id: questionOrChapter.id,
                    title: questionOrChapter.title,
                ...
                questionStep.value.questions.forEach(
                    (question, index) => {
                      ...
                      modifiedQuestion?.groups?.forEach(
                          (group) => {
                            if (group?.options) {
                      ...
                      if (
                          modifiedQuestion?.conditions
                      )
                        modifiedQuestion?.conditions?.forEach(
                            (condition) => {
                              condition.products =
                              ...
                              condition.conditionOptions.forEach(
                                  (option) => {
                                    option.option =
                                        this.allOptions[
                                            option.option
                      ...
                      modifiedQuestion.groups =
                          modifiedQuestion.groups.map(
                              (group) => {
                                return {
                                  ...group,
                ...
                if (modifiedQuestion?.conditions)
                  modifiedQuestion?.conditions?.forEach(
                      (condition) => {
                        condition.products =
                            condition.products.map(
            ...
            for (let key in this.allConditions) {
              ...
              condition.conditionOptions.forEach((option, index) => {
    ...
    exportJson() {
      try {
        ...
        link.click();
        URL.revokeObjectURL(url);
        let surveyCss = `div>>>.custom-action-grid {
    display: grid;
...
<html lang="en">
      ...
      try {
        ...
        for (let i = 0; i < this.steps.length; i++) {
          ...
          payload.questions.forEach((question) => {
            question.value.groups.forEach((group) => {
            if (group?.options) {
          ...
          try {
            if (this.steps[i].type === 'chapter') {
              await surveyChapterPutRequest(payload?.value ?? payload, payload?.value?.id ?? payload?.id)
            } else {
              await surveyQuestionPutRequest(payload?.value ?? payload, payload?.value?.id ?? payload?.id)
            }
          } catch (e) {
            let errorKey = "";
            if (
          ...
          }, this.$route.params.id ?? this.surveyId);
        }
        ...
        await surveyConfigurationsPostRequest({
    ...
    openConditionProductsModal(index) {
      this.selectedCondition =
          this.selectedQuestion.conditions[index];
    ...
    addOptionsCondition(index) {
      this.selectedQuestion.conditions[index].options.push({
        option: "",
        condition: "",
        operator: "&&",
        value:
    ...
    addCondition() {
      ...
      this.addConditionMixin(condition);
      this.selectedQuestion.conditions.push(condition);
    },
    ...
    addGroup() {
      ...
      this.selectedQuestion.groups.push(group);
    },
    addOption(index) {
      const uuid = uuidv4().replaceAll("-", "");
      const option = {
