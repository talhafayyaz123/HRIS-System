<template>
  ...
  >
    <div
      ...
      :key="
        'resource-' + (resource?.id ?? index + 'event-') + data.formattedDate
      "
      style="
        min-height: 40px;
    ...
    >
      <p
        v-if="!isWeekOrDayView"
        style="z-index: 9999"
        class="p-0 m-0 position-absolute"
      >
        {{ data.startDate?.getDate() }} -
        {{ data.endDate?.getDate() }}
      </p>
      <div
        v-for="(i, index) in !resource.rowExpanded ? cellDays : 0"
        :key="options.transitions ? `${view.id}-${data.content}-${i}` : i"
        :title="
          resource?.plannedStartDate + ' - ' + resource?.plannedFinishedDate
        "
        :style="`
          height: 15px;
          display: flex;
        ...
        @touchstart="
          !isDisabled && onCellTouchStart($event, splitsCount ? split.id : null)
        "
        @mousedown="
          !isDisabled && onCellMouseDown($event, splitsCount ? split.id : null)
        "
        @click="!isDisabled && onCellClick($event)"
        @dblclick="!isDisabled && onCellDblClick($event)"
        @contextmenu="
          !isDisabled && options.cellContextmenu && onCellContextMenu($event)
        "
        @dragenter="
          !isDisabled &&
            editEvents.drag &&
      ...
      <div
        ...
        style="height: 15px; border-left: 1px solid rgba(196, 196, 196, 0.25);"
        ...
        :key="options.transitions ? `${view.id}-${data.content}-${i}` : i"
        :title="
          resource?.events?.[0]?.startDate?.format('YYYY-MM-DD') +
          ' - ' +
          resource?.events?.[0]?.endDate?.format('YYYY-MM-DD')
        "
        :class="splitsCount && splitClasses(split)"
        :data-split="splitsCount ? split.id : false"
        column="column"
        ...
        @drop="onDrop($event, resource?.id ?? resource.belongsTo)"
      >
        <p v-if="!isWeekOrDayView" style="z-index: 9999999" class="p-0 m-0">
          {{ data.startDate?.getDate() }} -
          {{ data.endDate?.getDate() }}
        </p>
        <slot
        ...
        ></slot>
        <div
          class="vuecal__cell-events position-absolute"
          v-if="resource.events.length"
        >
          <event
            ...
            @removeEventReverse="$emit('removeEventReverse', $event)"
            :event="event"
            :cell="data"
            :all-day="allDay"
            ...
            "
            :overlaps-streak="
              splitsCount ? split.overlapsStreak : cellOverlapsStreak
            "
            ><template #event="{ event, view }">
              <slot name="event" :view="view" :event="event"></slot> </template
          ></event>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    checkIfCellDayInRange(date, plannedStartDate, plannedFinishedDate) {
      return (
        date >= new Date(plannedStartDate).getDate() &&
        date <= new Date(plannedFinishedDate).getDate()
      );
    },
    async onDrop(event, resourceId) {
      ...
      const resource = this.resources.find(
        (resource) => resource.id == event.dataTransfer.getData("resourceId")
      );
      ...
      eventTemp.plannedStartDate = eventTemp.startDate.format("YYYY-MM-DD");
      eventTemp.plannedFinishedDate = eventTemp.endDate.format("YYYY-MM-DD");
      try {
    ...
    checkIfInRange(startDate, endDate) {
      ...
      this.data.startDate.setHours(0, 0, 0, 0); //set the hours, minutes, seconds to 0
      this.data.endDate.setHours(0, 0, 0, 0); //set the hours, minutes, seconds to 0
      while (
        counter.setHours(0, 0, 0, 0) <= new Date(endDate).setHours(0, 0, 0, 0)
      ) {
        if (counter >= this.data.startDate && counter <= this.data.endDate) {
          isInRange = true;
          break;
        }
    ...
    setUpEventCreation(DOMEvent, startCursorY) {
      ...
      if (
        ...
        if (this.options.snapToTime) {
          let timeMinutes =
            this.timeAtCursor.getHours() * 60 + this.timeAtCursor.getMinutes();
          const plusHalfSnapTime = timeMinutes + this.options.snapToTime / 2;
          timeMinutes =
            plusHalfSnapTime - (plusHalfSnapTime % this.options.snapToTime);
  ...
  computed: {
    // returns an array with the date number ranging from startDate of the cell to the endDate
    cellDays() {
      let cellDays = [];
      this.data.startDate.setHours(0, 0, 0, 0); //set the hours, minutes, seconds to 0
      this.data.endDate.setHours(0, 0, 0, 0); //set the hours, minutes, seconds to 0
      let counter = this.data.startDate;
      while (counter <= this.data.endDate) {
        cellDays = [...cellDays, counter.getDate()];
        counter = counter.addDays(1);
      }
      return cellDays;
    },
    ...
    resourcesWithEventsFilteredByViewDates() {
      ...
      this.resources.forEach((resource) => {
        let events = resource.events.filter((event) => {
          ...
          if (this.checkIfInRange(startDate, endDate)) {
            dateExists = true;
          }
          return dateExists;
        });
