        ...
        >
          <all-day-bar
            v-if="
              showAllDayEvents &&
              hasTimeColumn &&
              (!cellOrSplitMinWidth || (isDayView && !minSplitWidth))
            "
            v-bind="allDayBar"
          >
          ...
          >
            ...
            <div class="vuecal__flex" row="row" grow="grow">
              <div style="width: 30%" class="vuecal__time-column">
                <table
                  style="
                    width: 100%;
                    display: block;
                ...
                >
                  ...
                  >
                    <tr
                      @click="resource.rowExpanded = !resource.rowExpanded"
                      style="
                        display: grid;
                    ...
                    >
                      ...
                      <td
                        v-if="resource.id != 0"
                        style="padding-left: 1rem; font-size: 0.8rem"
                      >
                    ...
                    >
                      ...
                      <td style="padding-left: 1rem; font-size: 0.8rem">
                        {{
                          (event?.actualStartDate)
                        }}
                      </td>
                      <td style="padding-left: 1rem; font-size: 0.8rem">
                        {{ (event?.actualFinishedDate) }}
                      </td>
                      <!-- <td
                                                class="overflow-text-gantt"
                                                style="
              ...
              >
                <div
                  ...
                  grow="grow"
                >
                  <slot name="week-number-cell" :week="getWeekNumber(i - 1)">{{
                    getWeekNumber(i - 1)
                  }}</slot>
                </div>
              </div>
              ...
              >
                ...
                <all-day-bar
                  v-if="
                    ...
                    ((isWeekView && cellOrSplitMinWidth) ||
                      (isDayView && hasSplits && minSplitWidth))
                  "
                  v-bind="allDayBar"
                >
                  <template #event="{ event, view }">
                    <slot name="event" :view="view" :event="event">
                      ...
                      <div
                        class="vuecal__event-content"
                        v-if="
                          event.content && !hasShortEvents && !isShortMonthView
                        "
                        v-html="event.content"
                      ></div>
                    </slot>
                ...
                >
                  <vuecal-cell
                    ...
                    :key="'cell-' + i + '-' + cell.formattedDate"
                    :monthsData="monthsData"
                    @addEvent="recalculatetaskHoursAndEmitAddEvent"
                    @addEventReverse="
                      recalculatetaskHoursAndEmitAddEventReverse
                    "
                    @removeEvent="recalculatetaskHoursAndEmitRemoveEvent"
                    @removeEventReverse="
                      recalculateresourcesHoursAndEmitRemoveEventReverse
                    "
                    :options="$props"
                    :edit-events="editEvents"
                    :data="cell"
                  ...
                  >
                    <template #cell-content="{ events, split, selectCell }">
                      ...
                      >
                        <div
                          class="split-label"
                          v-if="split && !stickySplitLabels"
                          v-html="split.label"
                        ></div>
                        <div
                          class="vuecal__no-event"
                          v-if="
                            !cellOrSplitHasEvents(events, split) &&
                            isWeekOrDayView
                          "
                        >
                          <!-- <slot name="no-event"
                    ...
                    <template #event="{ event, view }">
                      <slot name="event" :view="view" :event="event">
                        <div
                          style="left: 22px; top: 6px; position: absolute"
                          class="vuecal__event-title vuecal__event-title--edit"
                          v-if="editEvents.title && event.titleEditable"
                          contenteditable="contenteditable"
                          @blur="onEventTitleBlur($event, event)"
                          v-html="event?.taskHours?.[cell.formattedDate] ?? ''"
                        ></div>
                        <div
                          v-for="(hour, key) in event.taskHours"
                          :key="'hours-' + key + '-' + event.resourceId"
                          class="vuecal__event-title"
                          v-else-if="event.title"
                          v-html="''"
                        ></div>
                        <div
                          ...
                          v-if="
                            ...
                            !(
                              isMonthView &&
                              (event.allDay || showAllDayEvents === 'short')
                            ) &&
                            !isShortMonthView
                          "
                        >
                          {{ utils.date.formatTime(event.start, TimeFormat)
                          }}<span v-if="event.endTimeMinutes"
                            >&nbsp;-
                            {{
  ...
  data() {
    return {
      ...
      utils: {
        ...
        date:
          (this.disableDatePrototypes ? dateUtils.removePrototypes() : false) ||
          dateUtils,
        cell: null,
  ...
  methods: {
          ...
          if (this.hideWeekends) {
            ...
            if ([0, 6].includes(this.view.lastCellDate.getDay())) {
              const daysToSubtract =
                this.view.lastCellDate.getDay() === 0 && !this.startWeekOnSunday
                  ? 2
                  : 1;
      ...
      if (this.ready) {
        ...
        const params = {
          ...
          ...(this.isWeekView
            ? {
                week: ud.getWeek(
                  this.startWeekOnSunday ? ud.addDays(startDate, 1) : startDate
                ),
              }
            : {}),
        };
          ...
          case "week":
            if (monthOrYearChanged) {
              ...
              firstCellDate = ud[next ? "addDays" : "subtractDays"](
                ud.getPreviousFirstDayOfWeek(startDate, this.startWeekOnSunday),
                7
              );
            }
            break;
    ...
    eventDragCreation(e) {
      ...
      // If the event already exists change its start and end.
        ...
        if (this.snapToTime) {
          let timeMinutes =
            timeAtCursor.getHours() * 60 + timeAtCursor.getMinutes();
    ...
    updateSelectedDate(date) {
      ...
      if (date && date instanceof Date) {
        ...
        if (selectedDate)
          this.transitionDirection =
            selectedDate.getTime() > date.getTime() ? "left" : "right";
    ...
    getWeekNumber(weekFromFirstCell) {
      ...
      if (currentWeekNumber > 52) {
        return ud.getWeek(
          ud.addDays(this.view.firstCellDate, 7 * weekFromFirstCell + modifier)
        );
  ...
  computed: {
    ...
    cellOrSplitMinWidth() {
      ...
      if (this.hasSplits && this.minSplitWidth)
        minWidth =
          this.visibleDaysCount * this.minSplitWidth * this.daySplits.length;
    ...
    weekDays() {
      ...
      if (this.view.id === "month") {
        weekDays = [...weekDays.slice(7), ...weekDays];
      } else weekDays = [...weekDays.slice(-5), ...weekDays];
      ...
      weekDays = weekDays.slice(0).map((day, i) => ({
        ...
        hide:
          ...
          (this.hideWeekdays.length && this.hideWeekdays.includes(i + 1)),
      }));
    ...
    viewCells() {
      ...
      switch (this.view.id) {
        ...
        case "year": {
          let multiplier = 1;
          cells = Array.apply(null, Array(12 * this.resources.length)).map(
            (cell, i) => {
              const basicUnit = 12;
              ...
              if (startDate.getMonth() === ud.addDays(startDate, 30).getMonth())
                endDate = ud.addDays(startDate, 30);
              else if (
                startDate.getMonth() === ud.addDays(startDate, 29).getMonth()
              )
                endDate = ud.addDays(startDate, 29);
              else if (
                startDate.getMonth() === ud.addDays(startDate, 28).getMonth()
              )
                endDate = ud.addDays(startDate, 28);
              else if (
                startDate.getMonth() === ud.addDays(startDate, 27).getMonth()
              )
        ...
        case "month": {
          ...
          cells = Array.apply(
            null,
            Array((doesFifthWeekExist ? 5 : 4) * this.resources.length)
          ).map((cell, i) => {
            ...
            if ((i % (doesFifthWeekExist ? 5 : 4)) * 7 == 28) {
              if (
                ud.addDays(startDate, 3).getMonth() == firstCellDate.getMonth()
              ) {
                endDate = ud.addDays(startDate, 3);
              } else if (
                ud.addDays(startDate, 2).getMonth() == firstCellDate.getMonth()
              ) {
                endDate = ud.addDays(startDate, 2);
              } else if (
                ud.addDays(startDate, 1).getMonth() == firstCellDate.getMonth()
              ) {
            ...
            const isToday =
              !todayFound && ud.isToday(startDate) && !todayFound++;
