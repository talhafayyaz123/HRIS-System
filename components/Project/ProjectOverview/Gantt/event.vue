<template>
  <div :style="`display: grid; grid-template-columns: ${getTemplateColumns}`">
    <div
      v-for="i in (cellDays ?? [])"
      :key="'day-' + i"
      ...
      :class="eventClasses"
      :style="{ ...eventStyles, backgroundColor: checkIfCellDayInRange(i) ? 'rgba(160, 220, 255, 0.5)' : 'white' }"
      tabindex="0"
      @focus="focusEvent"
...
export default {
      ...
      handler: function () {
        ...
        this.$nextTick(() => {
          this.$refs.blockText.style.left =
            -1 * (this.$refs.blockText.getBoundingClientRect().width + 30) +
            "px";
        });
      },
      deep: true,
  ...
  methods: {
    checkIfCellDayInRange(date) {
      return (
        date >= this.event.startDate.getDate() &&
        date <= this.event.endDate.getDate()
      );
    },
    mouseover() {
      ...
      this.$nextTick(() => {
        this.$refs.blockText.style.left =
          -1 * (this.$refs.blockText.getBoundingClientRect().width + 30) + "px";
      });
    },
    ...
    async saveRelationship() {
      ...
      this.relationships.forEach(async (relationship) => {
        ...
        this.event.plannedStartDate = this.event.startDate.format("YYYY-MM-DD");
        ...
        if (
          ...
          relationshipAddedToTask.relationships.push({
            type: relationship.type === "blocks" ? "blockedBy" : "blocks",
      ...
      this.deletedRelationships.forEach(async (relationship) => {
        ...
        const relationshipIndex =
          relationshipRemovedFromTask.relationships.findIndex((r) => {
            return (
              r.type == relationship.type && e.taskId == relationship.taskId
            );
          });
        relationshipRemovedFromTask.relationships.splice(relationshipIndex, 1);
    ...
    onDragStart(e) {
      ...
      this.$refs.vuecalEvent.innerHTML = `<p style="font-size: 0.7rem;">${this.event.startDate
        .toString()
        .slice(0, 10)} - ${this.event.endDate.toString().slice(0, 10)}</p>`;
  ...
  computed: {
    /**
     * returns a column string with units based on cellDays
     */
    getTemplateColumns() {
      let columnString = "";
      for (let i = 0; i < this.cellDays.length; i++) {
        columnString += "1fr ";
      }
      return columnString;
    },
    grabRelationshipInfo() {
      ...
      for (let i = 0; i < this.relationships.length; i++) {
        relationshipText += `${this.enums[this.relationships[i].type]} ${
          this.events.find((event) => event.id == this.relationships[i].taskId)
            ?.name ?? ""
